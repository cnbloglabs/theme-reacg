import './index.scss'
import { useGiteeOptions, useGithubOptions } from '@acnb/options'
import { getCurrentPage } from '../../common/utils/cnblog'
import { loadScript, poll, randomProperty } from '../../common/utils/helpers'
import { fontUrl, foodIcons, icons } from './icons'
 ...
 */
function setGithub() {
  const { enable, color, url } = useGithubOptions()
  if (!enable) {
    return
  }
  const $githubIcon = `
 ...
 */
function setGitee() {
  const { enable, color, url } = useGiteeOptions()
  if (!enable) {
    return
  }
  const icon = `
...
function nav() {
  ...
  for (const { selector, icon } of items) {
    if ($(selector).length) {
